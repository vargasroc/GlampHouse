...
export function FilterSearchBox(props) {
    ...
    function dataForm(data) {
        if(data.cities !== undefined && data.category === undefined) {
            goTo(`/listar/${data.city}`);
        } else if (data.ciudades !== undefined && data.category !== undefined) {
            goTo(`/listar/${data.city}/${data.category}`)
        } else {
            goTo(`/listar//${data.category}`)
        }
    }
    return (
        <Container fluid className="bg-secondary py-3">
            <h2 className="text-center font-700 fs-2 text-white mb-3">Últimas tendencias en alojamientos.</h2>
